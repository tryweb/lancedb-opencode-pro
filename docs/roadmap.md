...
4. **DB row 型別與降級路徑可觀測性不足**
   - `as unknown as EpisodicTaskRecord` 使用面積大；embedding fallback 雖能保持可用，但部分路徑缺少明確診斷訊號。
...
- 建議搭配 skill / summary workflow，讓使用者更容易理解「這週學到了什麼」
...
- [ ] learning dashboard / weekly summary（Surface: Skill）
...
- [ ] `effectiveness_events` 的 TTL / archival（Surface: Plugin）
...
- [ ] Embedding fallback 可觀測性補強（保留降級、增加警示與統計）
...
### P0
1. learning dashboard / weekly summary（Surface: Skill）→ BL-030
...
7. success pattern 的 playbook surface 強化（Surface: Plugin + Skill）→ BL-040
...
17. Store repository 職責分離（Surface: Plugin）→ BL-042
18. Embedding fallback 可觀測性補強（Surface: Plugin + Docs）→ BL-047
19. weekly review automation 強化
20. trajectory relabeling / hindsight replay
...
1. **Learning dashboard / weekly summary（BL-030，Skill）**
