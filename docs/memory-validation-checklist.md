...
### 4.4 Benchmark Thresholds

This section defines the latency threshold policy for validation.
...
- These two values match the acceptance criteria in Phase 4.1.
- Search responsiveness is the most user-visible benchmark in the current release scope.

#### Informational Metrics

These values are reported in the benchmark output but do not fail the command.
...
- These values can be promoted to hard gates in a later release once the benchmark becomes more stable.
...
- The `full` profile remains available for deeper profiling and capacity analysis.
